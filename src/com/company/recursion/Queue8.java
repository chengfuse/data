...
public class Queue8 {
    //定义一共有多少个皇后
    int  max=8;
    //定义数组arrya，保存皇后放置位置的结果，比如arry={0,4,7,5,6,1,3}
...
//        System.out.println(Math.abs(-1));
    ...
    //编写一个方法，放置第n个皇后
    //特别注意；check 是每一次递归时，进入check中有 for(int i=0;i<max;i++),因此会有回溯
    ...
    private  boolean  judge(int  n){
        for (int i = 0; i < n; i++) {
            ...
            //Math.abs(n-i)==1  Math.abs(n-i)=Math.abs(array[n]-array[i])=1
